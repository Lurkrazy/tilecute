...
def gemm_ss(  # 128,256,32 gemm
    ...
    print("tCrA.layout", tCrA.layout)
    print("tCrB.layout", tCrB.layout)
    ...
    print("tCsA_p layout", tCsA_p.layout)
    print("tCsA_copy_view layout", tCsA_copy_view.layout)
    print("tCsB_copy_view layout", tCsB_copy_view.layout)
    ...
    for k in cutlass.range_dynamic(tCrA.shape[2]):
        ...
        print("mma A layout:", tCrA[None, None, k].layout)
        print("mma B layout:", tCrB[None, None, k].layout)
...
def gemm_f16f16f16_nn_kernel(
    ...
    smem_storage = smem.allocate_tensor(cutlass.Uint8, 49152, 128) # 1024 align
    ...
    print("tAsA layout", tAsA.layout)
    print("tBsB layout", tBsB.layout)
    print("tAgA layout", tAgA.layout)
    print("tBgB layout", tBgB.layout)

    C_local = cute.make_fragment(256, cutlass.Float16)
    ...
    #         dst = cute.recast_ptr(smem_storage.iterator + smem_offset, dtype = cutlass.Float16),
    #         src = gA.iterator + global_offset, 
    #         cp_size = 16,
    ...
    #     smem_offset = ((((((((((tidx) & 31) >> 3) * 4096) + (i_2 * 512)) + (((tidx) >> 5) * 128)) + ((((((tidx) & 7) >> 2) + (i_2 & 1)) & 1) * 64)) + (((((tidx) >> 6) + (((tidx) & 3) >> 1)) & 1) * 32)) + ((((((tidx) & 63) >> 5) + ((tidx) & 1)) & 1) * 16)) + 16384)
        
    #     global_offset = ((((i_2 * 4096) + (((tidx) >> 5) * 1024)) + ((bidx) * 256)) + (((tidx) & 31) * 8))

    #     cp_async_shared_global(
    #         dst = cute.recast_ptr(smem_storage.iterator + smem_offset, dtype = cutlass.Float16),
    #         src = gB.iterator + global_offset, 
    #         cp_size = 16,
    #         modifier = nvvm.LoadCacheModifierKind.CG  # enable L2 prefetch
    #     )
    ...
    for k in cutlass.range_dynamic(23): 
        ...
        #     smem_offset = (((((((((k + 1) & 1) * 16384) + ((((tidx) & 31) >> 3) * 4096)) + (i_4 * 512)) + (((tidx) >> 5) * 128)) + ((((((tidx) & 7) >> 2) + (i_4 & 1)) & 1) * 64)) + (((((tidx) >> 6) + (((tidx) & 3) >> 1)) & 1) * 32)) + ((((((tidx) & 63) >> 5) + ((tidx) & 1)) & 1) * 16)) + 16384

        #     global_offset = (((((k * 32768) + (i_4 * 4096)) + (((tidx) >> 5) * 1024)) + ((bidx) * 256)) + (((tidx) & 31) * 8)) + 32768
        ...
        sB = cute.make_tensor(cute.recast_ptr(smem_storage.iterator,dtype = cutlass.Float16)+(((k & 1) * 8192) + 8192), sB_layout)
    ...
    sB = cute.make_tensor(cute.recast_ptr(smem_storage.iterator, dtype = cutlass.Float16) + 16384, sB_layout)
    gemm_ss(sA, sB, C_local, tiled_mma)

    for i_5 in range(128):
        global_offset = ((((((((bidy) * 131072) + ((i_5 & 15) * 8192)) + ((((tidx) & 31) >> 2) * 1024)) + ((bidx) * 256)) + ((i_5 >> 4) * 32)) + (((tidx) >> 5) * 8)) + (((tidx) & 3) * 2))
...
def gemm_f16f16f16_nn(
    ...
    sA_layout = _make_smem_layout_AB(cutlass.Float16, a_major_mode, 128, (cta_tile_m, cta_tile_k, 2))
    sB_layout = _make_smem_layout_AB(cutlass.Float16, b_major_mode, 128, (cta_tile_n, cta_tile_k, 2))

    op = cute.nvgpu.warp.MmaF16BF16Op(cutlass.Float16, cutlass.Float16, (16, 8, 16), )
    num_warp = (1, 4, 1)
...
# print("a_torch", a_torch[:10,:10])
# print("b_torch", b_torch[:10,:10])
